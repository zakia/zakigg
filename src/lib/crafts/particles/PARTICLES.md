...
- If you draw _before_ clearing, last frame's image is still there (ghosting).
- If you draw _before_ updating, you see where things _were_, not where they _are_.
...
The `frame` callback is where _you_ decide what update + draw means for your specific effect.
...
A particle only knows about _itself_. It owns:
...
Think of a particle like a ball on a pool table. The ball knows its position, speed, and color. It does _not_ know about the table edges or the other balls — that's the environment's job.

### The Environment (the world)

The environment is everything _outside_ the particle that affects it. It's defined in the `setup` and `frame` callbacks of each effect.
...
Both effects follow the exact same lifecycle above. The _only_ difference is what happens inside `setup` and `frame`:

...
<div class="flex h-dvh flex-col">
	<nav
		class="bg-base/80 border-edge gap-s0 px-s1 py-s-1 z-10 flex items-center border-b backdrop-blur-sm"
	>
		{#each configs as cfg}
			<button
				class="btn text-s-1"
		...
		{/each}
		<span class="text-content-1 text-s-2 ml-auto">{current.description}</span>
	</nav>

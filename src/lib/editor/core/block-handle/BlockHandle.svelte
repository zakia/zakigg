<script lang="ts">
	...
	} from '$lib/editor/core/block-handle';
	import { runBlockCommand, type BlockCommand } from '$lib/editor/core/commands';
	import { BLOCK_TURN_TARGETS, type BlockTurnTarget } from '$lib/editor/core/blocks';
	...
	let grip = $state<HTMLButtonElement>();
	...
	function closeMenu({ restoreFocus = false } = {}) {
		...
		if (restoreFocus) void tick().then(() => grip?.focus());
	}

	function runAction(
		type: Extract<BlockCommand['type'], 'edit' | 'duplicate' | 'insert-below' | 'delete'>
	) {
		if (!editor || !pinnedTarget) return;

		runBlockCommand(editor, { type, pos: pinnedTarget.pos });
		closeMenu();
	}
	...
	function runTurnInto(turnTarget: BlockTurnTarget) {
		...
		runBlockCommand(editor, { type: 'turn-into', pos: pinnedTarget.pos, target: turnTarget });
		closeMenu();
	}
	...
	onDestroy(() => {
		if (menuOpen && editor) unlockBlockHandle(editor);
	});
</script>
...
<div class="block-handle" bind:this={root} data-block-handle-ui>
	{#if active}
		...
		<button
			...
			aria-controls={menuOpen ? 'editor-block-menu' : undefined}
		...
		{#if menuOpen && pinnedTarget}
			...
			>
				...
				{#if pinnedTarget.editLabel}
					<button type="button" class="menu-item" role="menuitem" onclick={() => runAction('edit')}>
				...
				<button
					...
					class="menu-item"
					role="menuitem"
					onclick={() => runAction('insert-below')}
				>

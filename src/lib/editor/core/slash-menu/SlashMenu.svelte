<script lang="ts">
	...
	import type { BlockPaletteItem } from '$lib/editor/core/blocks';
	...
	const activeOptionId = $derived(
		items[activeIndex] ? `${menuId}-option-${activeIndex}` : undefined
	);
	...
	$effect(() => {
		...
		void tick().then(() => {
			options?.querySelector(`#${activeOptionId}`)?.scrollIntoView({ block: 'nearest' });
		});
	});
</script>
...
{#if visible && items.length}
	...
	>
		...
		<div class="slash-options" bind:this={options}>

<script lang="ts">
	...
	$effect(() => {
		if (columns && columns.length > 0) {
			editableColumns = columns.map((col) => ({ name: col, removed: false, original: col }));
		}
	});
					...
					<div class="overflow-x-auto">
						<table class="min-w-full divide-y divide-gray-200">
							<thead class="bg-gray-50">
								<tr>
									{#each editableColumns as col, idx (col.original)}
										{#if !col.removed}
											<th
							...
							<tbody class="divide-y divide-gray-200 bg-white">
								{#each parsedData.slice(0, 20) as row, rowIndex (rowIndex)}
									<tr>
										{#each editableColumns as col (col.original)}
											{#if !col.removed}
												<td

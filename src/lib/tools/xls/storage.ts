...
export interface HistoryEntry {
	...
	size: number;
	meta?: Record<string, unknown>; // e.g., original filename, mimetype
}
...
function historyEntryToFile(entry: HistoryEntry): File | null {
	...
	const type =
		typeof entry.meta?.mimetype === 'string' ? entry.meta.mimetype : 'application/octet-stream';
	return new File([bytes], name, { type });
}

/** Convert a HistoryEntry (type 'paste') to parsed data and columns */
function historyEntryToParsedPaste(
	entry: HistoryEntry
): { columns: string[]; parsedData: Record<string, unknown>[] } | null {
	if (entry.type !== 'paste') return null;
	const rows = entry.data.split('\n').map((row) => row.split('\t'));
	if (!rows.length || !rows[0].length) return null;
	const columns = Array.isArray(entry.meta?.columns)
		? entry.meta.columns.map((column) => String(column))
		: rows[0];

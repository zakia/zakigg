	...
	onMount(() => {
		...
		const destroy = createParticleSystem(canvas, {
			...
			setup(_ctx, width, height) {
				...
				particles.push(
					new GravityParticle({
						x: cx, y: cy, radius: 30, mass: sunMass,
						vx: 0, vy: 0, config, canvas: canvas!
					})
				);
...
<div class="relative h-full w-full">
	...
	<div class="absolute right-4 top-4 flex flex-col gap-2">
		...
		<button
			class="btn variant-ghost"
			onclick={() => (config.show_trails = !config.show_trails)}
		>
			Trails: {config.show_trails ? 'on' : 'off'}
		</button>
		<label class="flex flex-col gap-1 text-s-2 text-content-1">

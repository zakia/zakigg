<script lang="ts">
	...
	onMount(() => {
		...
		function getColor() {
			return getComputedStyle(canvas!).color || 'white';
		}
			...
			frame(ctx, _width, _height) {
				...
				for (const p of particles) {
					for (const other of particles) {
						...
						if (dist < LINK_RADIUS) {
							ctx.strokeStyle = `oklch(from ${color} l c h / ${1 - dist / LINK_RADIUS})`;
							ctx.lineWidth = 2;
							ctx.beginPath();
							ctx.moveTo(p.x, p.y);
...
<div class="absolute inset-0 h-full w-full overflow-clip">
	<canvas bind:this={canvas} style="color: oklch(85% 0.18 var(--hue))"></canvas>
</div>

<script lang="ts">
  ...
  function isOctaveRelated(freq1: number, freq2: number): boolean {
    ...
    return Math.abs(ratio - 2) < 0.1 || Math.abs(ratio - 4) < 0.15 || 
           Math.abs(ratio - 0.5) < 0.05 || Math.abs(ratio - 0.25) < 0.05;
  }
  ...
  function getNeighborNotes(note: string, octave: number): { note: string; octave: number; freq: number }[] {
    if (note === "-") return [];
  ...
  function detectPitch(): void {
    ...
    if (frequency === -1) {
      ...
      const isOctaveChange = noteData.note === currentNoteCandidate && 
                             noteData.octave !== currentOctaveCandidate;
      ...
      if (isNoteChange) {
        ...
        const requiredStability = isDecaying() ? OCTAVE_STABILITY_THRESHOLD * 2 : OCTAVE_STABILITY_THRESHOLD;
  ...
  // --- Math Helpers ---
  function autoCorrelate(buffer: Float32Array<ArrayBuffer>, sampleRate: number): { frequency: number; confidence: number } {
    let sumOfSquares = 0;
    ...
    if (bestCorrelation > 0.8) { // Increased threshold for higher confidence
      return { 
...
<div class="tuner-container">
  {#if !tuner.isRunning}
    <button onclick={startTuner} class="btn variant-primary">
      Start Tuner
    </button>
  {:else}
...
<style>
  ...
  .needle {
    ...
    transition: left 0.12s ease-out, background-color 0.2s;
    z-index: 2;
  }
